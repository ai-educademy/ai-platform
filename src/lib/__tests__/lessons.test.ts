...
import { buildLessonPreview, getLessons, getLesson } from "@/lib/lessons";
...
describe("buildLessonPreview", () => {
  it("bounds the public excerpt and does not include the full gated body", () => {
    const lesson = {
      slug: "gated",
      title: "Gated Lesson",
      description: "A public summary for a gated lesson",
      order: 2,
      ...
      content: `# Gated Lesson

This is the public introduction. It explains the lesson context without exposing the paid walkthrough.

## First paid section

SECRET_PAID_BODY This section contains the detailed paid explanation that must not appear in the preview.

### Paid subsection

More paid material with examples, quiz answers, and implementation details.

## Final paid section

The complete lesson ends with another private explanation.`,
    };

    const preview = buildLessonPreview(lesson);

    expect(preview.introExcerpt).toContain("public introduction");
    expect(preview.introExcerpt).not.toContain("SECRET_PAID_BODY");
    expect(preview.introExcerpt).not.toContain("Final paid section");
    expect(preview.excerptWordCount).toBeLessThanOrEqual(
      Math.max(1, Math.floor(preview.wordCount * 0.15)),
    );
    expect(preview.outline).toEqual([
      { level: 2, title: "First paid section" },
      { level: 3, title: "Paid subsection" },
      { level: 2, title: "Final paid section" },
    ]);
  });
});
